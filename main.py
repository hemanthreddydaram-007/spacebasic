...
def get_target_date_str():
    ...
    SpaceBasic requires booking 1 day in advance before 9:30 PM.
    """
...
def run_automation():
    ...
    for user in users:
        ...
        # Options: "Non Veg", "Veg", or "Eggetarian"
        ...
        # Step 1: Fetch active meals from SpaceBasic
        try:
            res = requests.get(f"{GET_MEALS_URL}?date={target_date}", headers=headers)
            data = res.json()
            ...
            if not selected_lunch and lunch_opts:
                selected_lunch = lunch_opts[0] # Fallback
            if selected_lunch:
            ...
            if not selected_dinner and dinner_opts:
                selected_dinner = dinner_opts[0] # Fallback
            if selected_dinner:
